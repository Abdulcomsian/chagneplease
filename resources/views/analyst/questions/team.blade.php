...
@if($teamDetail->teamAiQuestion)
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          {{$teamDetail->teamAiQuestion->aiReport->report}}
        </div>
  ...
  @endif
    ...
    <div class="qs_title d-flex align-items-center">
      <img class="company_logo" src="{{asset('uploads/'.$teamDetail->company_logo)}}"  alt="sl">
      <div class="d-flex justify-content-between w-100">
        <div class="d-flex"><h2>{{$teamDetail->company_name}}</h2><img src={{asset('images/goal.png')}} class="goal-img"></div>
        @if($teamDetail->teamAiQuestion)
        <h4 class="mt-2">
    ...
    <div class="modal goal-modal" tabindex="-1" role="dialog">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            @forelse ($teamDetail->goals as $goal)
                <div>{{$goal->title}}</div>
            @empty
                No Goal Assigned!
            @endforelse
