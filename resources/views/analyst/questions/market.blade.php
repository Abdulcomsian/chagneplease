...
@section('main-content')
  @if($marketDetail->marketAiQuestion)
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          {{$marketDetail->marketAiQuestion->aiReport->report}}
        </div>
  ...
  @endif
    ...
    <div class="qs_title d-flex align-items-center">
      <img class="company_logo" src="{{asset('uploads/'.$marketDetail->company_logo)}}"  alt="sl">
      <div class="d-flex justify-content-between w-100">
        <div class="d-flex"><h2>{{$marketDetail->company_name}}</h2><img src={{asset('images/goal.png')}} class="goal-img"></div>
        @if($marketDetail->marketAiQuestion)
        <h4 class="mt-2">
          {{$marketDetail->marketAiQuestion->ai_rating}} / 10
        </h4>
        @endif
      </div>
    </div>
    ...
    @endphp
    <div class="qs_sec">
      @foreach($marketDetail->marketAnswer as  $answer)
      <div class="qs_box">
        <p>
      ...
      @if($marketDetail->marketAiQuestion)
        @php
          $aiQuestion = json_decode($marketDetail->marketAiQuestion->question);
          $aiAnswer = json_decode($marketDetail->marketAiQuestion->answer);
        @endphp 
    ...
    <div class="modal goal-modal" tabindex="-1" role="dialog">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            @forelse ($marketDetail->goals as $goal)
                <div>{{$goal->title}}</div>
            @empty
                No Goal Assigned!
            @endforelse
